...
df_windows = rolling_window(data, 0.8, 30)
random = []
predicted = []
real = []
for k in df_windows:
    p, r, rw = create_model(LinearRegression, RobustScaler, k[0], "ETTJ - 12m", k[1])
    random.append(rw)
    predicted.append(p[0])
    real.append(r)


rmse_linear = root_mean_squared_error(real, predicted)
