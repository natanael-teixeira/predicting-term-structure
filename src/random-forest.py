...
df_windows = rolling_window(data, 0.7, 30)
random = []
predicted = []
real = []
for k in df_windows:
    p, r, rw = create_model(RandomForestRegressor, RobustScaler, k[0], "ETTJ - 1m", k[1], n_estimators=1000)
    random.append(rw)
...
rmse_randomforest = root_mean_squared_error(real, predicted)
...
dm_rw = dm_test(real, predicted, random, h = 30, crit="MSE")
print (dm_rw)
